...
function getRefreshToken($client_id, $client_secret, $url)
{
    ...
    curl_setopt($ch, CURLOPT_POSTFIELDS, "grant_type=$grant_type&code=$code&redirect_uri=$redirect_uri");
    $headers = array();
...
function getAccessToken($client_id, $client_secret, $url)
{
    ...
    $refresh_token = defined('REFRESH_TOKEN') ? REFRESH_TOKEN : getRefreshToken($client_id, $client_secret, $url);
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $url);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    curl_setopt($ch, CURLOPT_POST, 1);
